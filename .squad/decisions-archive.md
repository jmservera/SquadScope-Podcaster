...
### 2026-09-08: Security review approved for Podcaster #665 Spotify video draft promotion
**By:** Hermes (Security)
**What:** Independent security review for PR #665 found no blocking issues in `promote_spotify_video_draft()` or its distribution wiring. The review confirmed credential-safe logging, independent video live-publish gating, immutable W35 denylist enforcement for anchor IDs `124658107`, `124658398`, and `124662333`, explicit-anchor/no-duplicate behavior, state-based idempotency, and at-most-one publish POST per call. The review also confirmed fail-safe defaults and sanitized terminal telemetry for `spotify_video_publication_terminal`.
**Why:** The implementation keeps Spotify video promotion fail-closed by default, separates video authorization from audio publishing controls, blocks protected W35 anchors before auth or API calls, and avoids secret leakage while preserving operator-visible terminal outcomes. Hermes approved the change for merge with no additional recommendations.

## 2026-09-08: Hermes security review for PR #667

🟢 Verdict: No security concerns found in PR #667.

Findings:
- None.

Checks performed:
- No plaintext credentials or secrets added.
- New defaults are safe: `SPOTIFY_VIDEO_ALLOW_LIVE_PUBLISH='false'` and `SPOTIFY_VIDEO_PUBLISH_MODE='draft'`.
- Env var names match implementation: `SPOTIFY_VIDEO_ALLOW_LIVE_PUBLISH` and `SPOTIFY_VIDEO_PUBLISH_MODE`.
- No new secret exposure or inappropriate Bicep module secret access detected.
- PR review threads requiring resolution: No. (`gh pr view 667 --json reviews,reviewDecision` returned no reviews and no review decision.)

Safe to merge: Yes.

### 2026-09-08T17:07Z: W37 Spotify video canary result

**By:** Podcaster Squad (Bender/canary execution)
**What:** Ran canary against W37 Spotify video `anchor_id` `125401976` with authentication and full gate enabled.

**Outcome:** `manual_handoff_required` — provider constraint confirmed.
- Authentication: `SP_DC`/`SP_KEY` valid ✅
- `GET /v3/episodes/125401976` → HTTP 400 (state query failed)
- `POST /v3/episodes/125401976/publish?isMumsCompatible=true` → HTTP 404 (not found)

**Decision:** The Spotify Anchor v3 API does not support automated video draft promotion for this `anchor_id`/episode type. The PRD-specified honest manual handoff is the correct outcome. No silent success. No duplicate upload. Gate remains disabled (`false`/`draft`) by default.

**Manual action required:** Operator must publish W37 video via Spotify for Creators UI (`https://podcasters.spotify.com` → Episodes → Drafts → W37 episode → Publish video).

**Do NOT:** Publish W37 audio (`125398950`) or touch W35 drafts (`124658107`, `124658398`, `124662333`).

### 2026-09-08: Spotify video publication API confirmed unsupported for video anchor IDs

**Canary result:** POST `/v3/episodes/{anchor_id}/publish?isMumsCompatible=true` returns HTTP 404 for VIDEO anchor IDs (confirmed with clean preflight GET 200 for `anchorId=125401976`).

**Finding:** The Anchor v3 publish endpoint is confirmed audio-only. Video episodes cannot be promoted via this endpoint. The implementation correctly returns `terminal_state: manual_handoff_required`.

**Resolution:** Per PRD `docs/prd-spotify-video-live-publish.md`, the W37 operator action is required: publish `anchorId=125401976` manually via Spotify for Creators dashboard.

**Gates remain:** `SPOTIFY_VIDEO_ALLOW_LIVE_PUBLISH=false`, `SPOTIFY_VIDEO_PUBLISH_MODE=draft` in production video job.

**Future:** Gates stay disabled until a valid video publication API is discovered. The promote helper is production-ready for the GET-state path; the POST path needs a different endpoint when/if Spotify documents one.

---
### 2026-09-08: Anchor v3 API response format for episode state GET

**Finding:** GET `/v3/episodes/{anchor_id}?userId=...` returns `{"episodes": [{"episodeId": N, "isPublished": bool}], "audios": [...], "users": [...]}`. The episode ID field is `episodeId` (not `id`), and only `isPublished` is returned (no `isDraft` field). This is now handled by `_extract_state()` (PRs #669 and #670).
...
# Hermes Security Review: PR #666 Video Gate Wiring + Error Surface
...
|---|----------|------|---------|
| 1 | Medium | Backward compatibility | `upload_to_spotify_episode(..., return_episode_id=True)` now returns a 3-tuple `(ok, episode_id, promote_state)` instead of the previous 2-tuple, so any existing caller outside the updated `distribute_video()` path that still unpacks two values will raise at runtime. The repo’s internal caller was updated, but the API contract is no longer backward-compatible. |
...
- Bicep defaults are correct: `spotifyVideoAllowLivePublish='false'` and `spotifyVideoPublishMode='draft'` in both `infra/main.bicep` and `infra/modules/aca.bicep`.
- ACA wiring is correct: `SPOTIFY_VIDEO_ALLOW_LIVE_PUBLISH` is sourced from the parameter, not hardcoded.
- Workflow wiring is safe: values come from `vars.*`, and both env export / deploy parameter paths use `:-false` and `:-draft` fallbacks; no default path enables video live publish on `main`.
- Error surfacing is correct: live-promotion failures become `result.errors`, `result.status` becomes `partial` not `completed`, and `spotify_video_is_published=False`; draft gate denial does not add an error.
- Promotion is skipped when `anchor_episode_id is None`.
- I found no raw `sp_dc`, `sp_key`, or promote `details` logging added by this change; terminal state stores enum-like status strings, not credentials.
- Gate independence looks correct: video promotion uses `SPOTIFY_VIDEO_ALLOW_LIVE_PUBLISH`, separate from `SPOTIFY_ALLOW_LIVE_PUBLISH`.

## Validation

- `pytest -q tests/test_video_distribution.py -k 'spotify or return_episode_id' tests/test_publish.py -k 'video_gate or PromoteSpotifyVideoDraft' tests/test_deploy_workflow.py`
- Result: `20 passed`
...
# Hermes re-review: PR #668 preflight fix

Verdict: 🟡
Safe to merge: Yes from a security/privacy standpoint; not fully review-clean because two non-security threads remain open.

Findings:
- Confirmed in `podcaster/publish.py` line 467 that `_resolve_legacy_ids()` now logs `"***"` instead of raw `user_id`.
- Reviewed PR #668 diff; the masking change is present in the second patch.
- Searched `publish.py` for `user_id|userId` in logging/printing paths and found no other unmasked `userId` logging in the promote path. The only logging hit tied to the remediated issue is the masked line.

Review threads:
- Resolved safely: `PRRT_kwDOSzuis86gV5z4` (raw `userId` INFO-log exposure) — fixed.
- Still open: `PRRT_kwDOSzuis86gV50p` (`_get_episode_publication_state` docstring does not explain the new `user_id` parameter).
- Still open/outdated: `PRRT_kwDOSzuis86gV51G` (test does not explicitly assert `userId` forwarding into the state-read call).

Conclusion:
- Previous 🔴 security/privacy finding is remediated.
- No additional unmasked `userId` logging was found in the promote path.
- Remaining open threads are correctness/documentation follow-ups, not security blockers.

### 2026-09-08T20:34:59Z: Operator decision — ACR Basic, private networking deferred

**By:** jmservera (operator, via ask_user)
**What:** Keep ACR Basic SKU and `deployVnet=false` posture indefinitely. No ACR Premium upgrade, no new private link endpoints. Estimated deferred cost: ~$175/mo.
**Why:** Cost-benefit decision — current posture (strong auth, public endpoints, Storage already private via pre-existing VNet) is sufficient.
**Effect on #598:** Closed as intentionally deferred. Code available but activation not authorized.
**Effect on #34:** Private mode partially satisfied (Storage/ACA VNet). ACR/OpenAI private endpoints not deployed. Do not claim full private-by-default AC satisfied.
**Constraint:** Any future `deployVnet=true` deploy requires fresh explicit operator cost approval.

{"reviewDecision":"","reviews":[]}

diff --git a/podcaster/publish.py b/podcaster/publish.py
index 4bd87f7..7a9bc27 100644
--- a/podcaster/publish.py
+++ b/podcaster/publish.py
...
@@ -1591,7 +1592,7 @@ def _extract_state(payload: Any) -> bool | None:
             session,
             "GET",
             url,
-            params=_mums_params(),
...
index 5b95387..ae3d584 100644
--- a/tests/test_publish.py
+++ b/tests/test_publish.py
@@ -1476,6 +1476,7 @@ def _patch_dependencies(self, monkeypatch, pub, *, states=None, publish_side_eff
...
@@ -1608,7 +1609,7 @@ def test_unknown_readback_state_is_terminally_unknown(self, monkeypatch):
...
@@ -1619,7 +1620,7 @@ def test_unknown_readback_state_is_terminally_unknown(self, monkeypatch):
...
### 2026-09-08: Podcaster #665 delivered — Spotify video draft promotion
**By:** Squad (Coordinator) for jmservera
**What:** promote_spotify_video_draft() implemented in podcaster/publish.py, tests added, Hermes security review Green. PRD #664 and implementation #665 both merged. SPOTIFY_VIDEO_ALLOW_LIVE_PUBLISH defaults false; W35 IDs 124658107/124658398/124662333 permanently blocked.
**W37:** anchorId=125401976 remains DRAFT; capability canary required before enabling automation. Manual handoff per PRD Section "W37 Operator Action."
**Why:** PRD-approved implementation of safe video draft promotion gate (jmservera/SquadScope-Podcaster#664).

### 2026-09-21T21-25-51: PR #682 remediation and delivery constraints
**By:** jmservera
**What:** PR #682 remediation and delivery constraints
**References:** PR #682, branch squad/video-stage-budget-redesign
**Why:** Remediate every currently unresolved review thread on PR #682 with surgical fixes, focused regression tests, required documentation, and full relevant validation. Work only in the existing isolated worktree/branch squad/video-stage-budget-redesign. Preserve all safety/publication gates and do not weaken, skip, xfail, or scope away tests or CI/security checks. Do not merge, deploy production, create Azure spend, or mutate production state. Resolve a review thread only after its finding is demonstrably fixed. Leave the PR ready for operator review only.



## Archived 2026-09-22T09-57-58.635: Tier 2 archival (entries >7 days old, file >50KB)
