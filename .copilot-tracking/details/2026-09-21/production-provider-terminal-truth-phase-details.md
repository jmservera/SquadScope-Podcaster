...
| P05-T03 | Notification-loss rejection repaired at exact source head `a91fce9`; broader task remains open for operator/delivery gates | Durable `reserved`/`sending`/`accepted` notification state preserves one initial-send winner. Ambiguous acceptance is non-replayable, while every initial provider leg and migrated legacy consumed intent has scheduled reconciliation. Focused `7`, relevant suites `364`, Ruff check/format, changed-file compilation, and diff safety passed. PR #684 remains draft/open and blocked by operator-only #682. |
