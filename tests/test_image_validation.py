...
def _bmp_file_header(dib_size: int) -> bytes:
    """The 14-byte BMP file header plus the DIB header size field."""
    return (
        b"BM"
        + struct.pack("<I", 0)  # file size (not validated: the read is capped)
        + struct.pack("<HH", 0, 0)  # reserved words, required to be zero
        + struct.pack("<I", (14 + dib_size) & 0xFFFFFFFF)  # pixel-array offset
        + struct.pack("<I", dib_size)  # DIB header size
    )


def _bmp(width: int, height: int, dib_size: int = 40) -> bytes:
    """A BITMAPINFOHEADER-family BMP: 32-bit signed dimensions."""
    return _bmp_file_header(dib_size) + struct.pack("<ii", width, height) + b"\x00" * 8


def _bmp_core(width: int, height: int) -> bytes:
    """A BITMAPCOREHEADER (OS/2 1.x) BMP: 16-bit unsigned dimensions."""
    return _bmp_file_header(12) + struct.pack("<HH", width, height) + b"\x00" * 8
...
class TestBmpDibHeaderVariants:
    """``"BM"`` is two ASCII bytes — the header must carry the rest of the proof.

    Accepting any payload that starts with ``BM`` and reading 32-bit dimensions
    at a fixed offset did two wrong things at once: it let ordinary text past the
    "bytes decide" boundary and into ffmpeg, and it mis-parsed a legitimate
    ``BITMAPCOREHEADER`` file, whose 16-bit dimensions sit where a
    ``BITMAPINFOHEADER`` keeps the low half of its width.
    """

    def test_core_header_dimensions_are_16_bit(self):
        assert sniff_image(_bmp_core(10, 20)) == ImageInfo("bmp", 10, 20)

    def test_core_header_is_not_read_as_32_bit(self):
        """The exact mis-parse: 16-bit 10x20 read as 32-bit is 1310730x0."""
        data = _bmp_core(10, 20)
        misparsed_width, misparsed_height = struct.unpack("<ii", data[18:26])
        assert (misparsed_width, misparsed_height) != (10, 20)
        assert sniff_image(data).width == 10

    @pytest.mark.parametrize("dib_size", [16, 40, 52, 56, 64, 108, 124])
    def test_every_defined_32_bit_variant_is_accepted(self, dib_size):
        assert sniff_image(_bmp(10, 20, dib_size=dib_size)) == ImageInfo("bmp", 10, 20)

    def test_top_down_bitmap_still_accepted(self):
        assert sniff_image(_bmp(10, -20)) == ImageInfo("bmp", 10, 20)

    @pytest.mark.parametrize("dib_size", [0, 1, 11, 13, 39, 41, 0xFFFFFFFF])
    def test_undefined_dib_header_size_is_rejected(self, dib_size):
        with pytest.raises(InvalidImageError) as excinfo:
            sniff_image(_bmp(10, 20, dib_size=dib_size))
        assert excinfo.value.reason == "malformed_image"

    def test_nonzero_reserved_fields_are_rejected(self):
        data = bytearray(_bmp(10, 20))
        data[6:10] = struct.pack("<HH", 1, 0)
        with pytest.raises(InvalidImageError) as excinfo:
            sniff_image(bytes(data))
        assert excinfo.value.reason == "malformed_image"

    def test_pixel_offset_inside_the_header_is_rejected(self):
        data = bytearray(_bmp(10, 20))
        data[10:14] = struct.pack("<I", 20)  # < 14 + 40
        with pytest.raises(InvalidImageError) as excinfo:
            sniff_image(bytes(data))
        assert excinfo.value.reason == "malformed_image"
    ...
    @pytest.mark.parametrize(
        "payload",
        [
            b"BM report 2026: revenue up, see the attached spreadsheet for detail",
            b"BM" + b"\x00" * 64,
            b"BMP file follows this line, generated by an internal tool at 09:00",
        ],
    )
    def test_text_starting_with_bm_is_not_an_image(self, payload):
        """Non-image bytes must never reach ffmpeg on a two-byte coincidence."""
        with pytest.raises(InvalidImageError):
            sniff_image(payload)
        assert is_valid_image(payload) is False

    def test_truncated_between_the_two_headers_is_rejected(self):
        with pytest.raises(InvalidImageError) as excinfo:
            sniff_image(_bmp(10, 20)[:17])
        assert excinfo.value.reason in ("malformed_image", "not_an_image")

    def test_core_header_truncated_before_dimensions_is_rejected(self):
        with pytest.raises(InvalidImageError) as excinfo:
            sniff_image(_bmp_core(10, 20)[:21])
        assert excinfo.value.reason == "malformed_image"

    def test_bomb_dimensions_still_rejected_in_both_variants(self):
        for sample in (_bmp(60000, 60000), _bmp_core(60000, 60000)):
            with pytest.raises(InvalidImageError) as excinfo:
                sniff_image(sample)
            assert excinfo.value.reason == "image_too_large"

    def test_zero_dimensions_rejected_in_both_variants(self):
        for sample in (_bmp(0, 20), _bmp_core(10, 0)):
            with pytest.raises(InvalidImageError) as excinfo:
                sniff_image(sample)
            assert excinfo.value.reason == "invalid_dimensions"

    def test_variant_table_matches_the_documented_minimum(self):
        """The declared per-format minimum is the smallest variant's, not the largest."""
        assert min(BMP_DIB_HEADER_DIMENSIONS) == 12
        assert BMP_DIB_HEADER_DIMENSIONS[12] == "16"
        assert set(BMP_DIB_HEADER_DIMENSIONS.values()) == {"16", "32"}
        assert FORMAT_HEADER_MIN_BYTES["bmp"] == 22
