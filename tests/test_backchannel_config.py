...
def test_defaults_are_disabled_and_conservative():
    ...
    assert cfg.max_gap_seconds == 30.0
...
def test_clamped_gain_db_window():
    assert BackchannelConfig(gain_db=-12).clamped_gain_db == -12.0
    assert BackchannelConfig(gain_db=-10).clamped_gain_db == -10.0
    assert BackchannelConfig(gain_db=-5).clamped_gain_db == -10.0  # above ceiling
    assert BackchannelConfig(gain_db=-30).clamped_gain_db == -18.0  # below floor
