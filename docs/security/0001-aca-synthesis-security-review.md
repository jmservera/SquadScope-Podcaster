...
| ACA API app managed identity | Storage Queue Data Contributor | Storage account | Existing enqueue path grant. It is not RG/subscription-wide, but broader than a single queue; consider a future least-privilege follow-up once queue dependencies are separated. | Reviewed, unchanged to avoid touching deploy/smoke path. |
