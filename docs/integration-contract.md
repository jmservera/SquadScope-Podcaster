...
- `podcast_config.dog_logo` (optional object): Configures a DOG (Digital On-Screen Graphic) logo watermark overlaid on the **main content** of the generated video (never on the intro/outro bumpers, which carry their own branding). When absent, no watermark is applied (graceful). When **present**, a watermark is guaranteed: the Claracle logo is bundled in the synthesis image (`assets/images/claracle.jpeg`), canonical Claracle URLs resolve to it with no network fetch, and a failed remote download falls back to it. If even the bundled asset is missing from the image the composition fails explicitly instead of shipping an unbranded episode. Sub-fields:
  - `url` (string, default `https://raw.githubusercontent.com/jmservera/SquadScope/main/assets/images/claracle.jpeg`): Logo image URL. Canonical Claracle URLs (`claracle.com/images/claracle.jpeg`, `claracle.com/assets/images/claracle.jpeg`, and the SquadScope `raw.githubusercontent.com`/`github.com` variants) are served from the bundled asset. Any other URL is downloaded through the SSRF-guarded fetcher, validated as an image of at most 16 MiB, and cached at composition time.
